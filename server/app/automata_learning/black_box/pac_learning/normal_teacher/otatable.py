...
import copy
from app.automata_learning.black_box.pac_learning.normal_teacher.ota import Timedword, ResetTimedword, dRTWs_to_lRTWs, lRTWs_to_DTWs
...
class OTATable(object):
    ...
    def is_consistent(self):
        """
            ...
            (if tws1,tws2 in S U R, if a in sigma* tws1+a, tws2+a in S U R and row(tws1) = row(tws2), then row(tws1+a) = row(tws2+a))
        """
        new_a = None
        new_e_index = None
            ...
            for j in range(i + 1, len(table_element)):
                if table_element[i].row() == table_element[j].row():
                    ...
                    for element in table_element:
                        if len(element.tws) == len(table_element[i].tws) + 1 and element.tws[:-1] == table_element[i].tws:
                            new_element1 = Element([element.tws[-1]], [v for v in element.value], [])
                            temp_elements1.append(new_element1)
                        if len(element.tws) == len(table_element[j].tws) + 1 and element.tws[:-1] == table_element[j].tws:
...
def add_ctx_normal(dtws, table, ota):
    ...
    for ctx in ctxs:
        ...
        normalize(local_tws)
        if check_guessed_reset(local_tws, table):
