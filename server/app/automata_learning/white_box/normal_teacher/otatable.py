...
import copy
from app.automata_learning.white_box.normal_teacher.ota import Timedword, ResetTimedword, dRTWs_to_lRTWs, lRTWs_to_DTWs
...
class OTATable(object):
    ...
    def is_consistent(self):
        """
            Determine whether the table is consistent (if tws1,tws2 in S U R, if a in sigma* tws1+a, tws2+a in S U R and row(tws1) = row(tws2), then row(tws1+a) = row(tws2+a))
        """
        new_a = None
        new_e_index = None
...
def guess_resets_in_suffixes(table, to_guess=True):
    """
        Given a table T, before membership querying, we need to guess the reset in the suffixes. This method is for one element in S or R.
    """
