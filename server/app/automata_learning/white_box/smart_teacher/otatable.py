...
import copy
from app.automata_learning.white_box.smart_teacher.ota import Timedword, ResetTimedword, is_valid_rtws, dRTWs_to_lRTWs
...
class Element(object):
    ...
    def __init__(self, tws=None, value=None):
        if tws is None:
            tws = []
        if value is None:
            value = []
...
class OTATable(object):
    ...
    def is_prepared(self, ota):
        ...
        if flag_closed and flag_consistent and flag_evid_closed:
            return True
        else:
            return False
...
def get_TW_delay_zero(tws, action, ota):
    ...
    """
    local_tws = tws
    if not tws[len(local_tws) - 1].reset:
